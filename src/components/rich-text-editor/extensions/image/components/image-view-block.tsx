...
interface UseImageActionsNode extends Node {
  attrs: {
    src: string
    alt?: string
  }
...
function isImageSrc(value: unknown): value is ImageSrc {
  return typeof value === "object" && value !== null && "src" in value && typeof (value as ImageSrc).src === "string"
}
...
export const ImageViewBlock: React.FC<NodeViewProps> = ({
  ...
  const { isLink, onView, onDownload, onCopy, onCopyLink, onRemoveImg } =
    useImageActions({
      editor,
      node: node as unknown as UseImageActionsNode,
      src: imageState.src,
            ...
            <div className="relative h-full">
              ...
              >
                <img
                  ...
                  alt={(node.attrs as ImageNodeAttributes).alt ?? ""}
                  title={(node.attrs as ImageNodeAttributes).title ?? ""}
                  id={(node.attrs as ImageNodeAttributes).id}
                />
              </ControlledZoom>
            </div>
